...
- Oklahoma: Demographic and age data too messy to parse cleanlys
...
get_oklahoma = function() {
  ...
  return(list(
    ...
    comments = list(
      "Demographic and age data available, but is displayed in undesirable
      format for pdf scraping. Too fragile."
    )
  ))
}
...
get_mississippi= function() {
  ...
  summary_vals = read_html(site_url) %>% 
    html_nodes("body #pageContainer #pageContent #article .msdh table") %>% 
    html_table() %>% .[[1]] %>% 
    tail(1) %>% unlist() %>% .[-1] %>% 
  ...
  return(list(
    data = as_tibble(skeleton),
    comments = list(
      "Age data only available in images on site"
    )
  ))
...
```{r}
# Note to self: if we try to  scrape for before or at April 20
# the format of the pdf changed
...
```{r}
url1 = "https://ghs.guam.gov/jic-release-no-118-end-day-results-zero-test-positive-covid-19-dphss"
...
get_guam = function(url) {
  ...
  skeleton = skeleton_table(old_cols)
